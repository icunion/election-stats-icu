import React from 'react'

import './styles/common.scss'
import config from './config'

export { default as Banner } from './blocks/Banner'
export { default as DashboardLE2022 } from './pages/le2022/Dashboard'
export { default as StepByStepLE2022 } from './pages/le2022/StepByStep'
export { default as DashboardLE2023 } from './pages/le2023/Dashboard'
export { default as StepByStepLE2023 } from './pages/le2023/StepByStep'
export { default as DashboardLE2024 } from './pages/le2024/Dashboard'
export { default as StepByStepLE2024 } from './pages/le2024/StepByStep'
export { default as configureStore } from './arithmospora/store'

export * from './hooks'
